...
function with_key_env(f::Function, key::String)
    key_env_name = FredData.KEY_ENV_NAME
    withenv(key_env_name => key) do
        f()
    end
end

function with_key_file(f::Function, key::String)
    key_file_name = FredData.KEY_FILE_NAME
    mktempdir() do tmpdir
        open(joinpath(tmpdir, key_file_name), "w") do f
            write(f, key)
        end
        withenv(key_env_name => nothing,
                "USERPROFILE" => tmpdir,
                "HOME" => tmpdir, )
        do
            f()
...
@testset "Client creation fails with invalid/missing key" begin
    bad_key_short = repeat("c", convert(Int, round(FredData.API_KEY_LENGTH/2)))
